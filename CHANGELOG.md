...
- **Unmappable constructs are refused by name.** A tool call the conversation never answers, anything between a call and its result, an orphan result, `strict` without `additionalProperties: false`, and arguments that are not valid JSON each return a 400 naming the construct rather than being approximated or forwarded for the provider to reject opaquely. (`UnmappableError`, `docs/evidence/known-limitations.md` §2.8)
