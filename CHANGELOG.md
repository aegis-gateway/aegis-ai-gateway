...
### Added
- **Permitted requests are attested.** `audit_events` gains `request_complete` for every request that was served and `provider_failure` for every request that passed each gate and then failed upstream. `audit_events` is the only table the sealer covers, so before this the Merkle chain attested what the gateway refused and nothing about what it allowed through. Emitted exactly once per request on both the buffered and streamed paths. (`internal/audit/logger.go`, `internal/gateway/handler.go`, `internal/gateway/streaming_enhanced.go`)

  The row carries identity, endpoint, status, the configured provider key, the concrete model from `configs/models.yaml`, and whether the request was streamed. It does **not** carry the requested alias, classification, latency or token counts: `audit_events` has no column for them, and adding one changes the leaf-hash field set and requires `hash_schema_version=3`. Those five facts remain in `usage_records`, joinable on `request_id` and **not attested**. See [ADR 0011](docs/adr/0011-tool-names-wait-for-a-hash-schema-bump.md) and `docs/evidence/known-limitations.md` §2.13.

  `provider_failure.reason` takes one of six enumerated stage constants. It is covered by the leaf hash and sealed, so a Go error string or a provider's error text there would be a retention violation; `validFailureStage` refuses anything else and records `unknown` rather than dropping the event.
- **`aegis_audit_write_failures_total{event_type,reason}`.** No counter for dropped audit writes existed. A dropped write leaves no row and, because `BIGSERIAL` allocates only on a successful insert, no id gap either, so the sealer seals a contiguous run and reports a healthy chain over an incomplete record. This counter is the only signal that it happened. Alert on any increase. (`internal/telemetry/metrics.go`)
- **Allow-path zero-retention canaries.** `TestNoPayload_AllowPathCanary` and `TestNoPayload_AllowPathCanaryStreaming` assert against a live gateway that a served request writes exactly one `request_complete` row and that neither the prompt nor the response text appears in any audit row. The existing canary covered only the refusal path, which is now the minority of what gets sealed. CI checks all three by name. (`internal/audit/allow_no_payload_integration_test.go`)
...
### Fixed
- **The per-key model allowlist is enforced on the completion path.** `AuthInfo.AllowedModels` was populated from the database and read only by `GET /v1/models`, so a key restricted to `aegis-fast` was listed one alias and served any of them, then billed for it. `docs/COMPLIANCE-MAPPING.md` cites this as the SOC 2 CC6.1 logical access control, so it was a capability claimed and not implemented. An empty allowlist still means unrestricted, which is what `cmd/keygen` writes for every key it issues. A refused request gets the same 503 envelope as the classification-ceiling refusal and writes an `auth_failure` row carrying the caller's real organization. (`internal/auth/context.go`, `internal/gateway/handler.go`)
- **A stream that failed is no longer billed as a success.** `StreamMetrics.Outcome` drove the audit event while the Prometheus counter and the usage record still hardcoded 200, so a stalled stream was sealed as a 504 and recorded as a 200 under one `request_id`. All three sinks now take `StreamOutcome.HTTPStatus()`. (`internal/gateway/streaming_enhanced.go`)
- **An exhausted-retry 5xx is attested as a provider HTTP error, not as unreachable.** `retry.Executor` returns the last response alongside `ErrMaxRetriesExceeded`, so a provider that answered 502 three times arrived with a non-nil response and a non-nil error and was sealed as `provider_unreachable`, while the streaming path sealed the same rejection as `provider_http_error`. That response is also now closed, which returns the connection to the pool instead of leaking one per exhausted retry. (`internal/gateway/handler.go`)
- **A provider HTTP error is attested as one.** The non-streaming path recorded every `TransformResponse` error as `provider_response_invalid`, including an ordinary provider 429 or 500, while the streaming path already sealed those as `provider_http_error`. The attested stage for one provider rejection therefore depended on whether the caller asked to stream. (`internal/gateway/handler.go`)
- **A restricted key could not put arbitrary text in the sealed audit record.** The allowlist check runs before `ResolveRoute` by design, so nothing had rejected an unconfigured alias before it was written to `audit_events.model`, a column the leaf hash covers. A caller holding a restricted key could seal up to 128 characters of `[a-zA-Z0-9._:-]` into the attested trail. The allowlist is now enforced only on an alias configured in `configs/models.yaml`; anything else falls through to `ResolveRoute` and writes no audit row. (`internal/gateway/handler.go`, `TestChatCompletions_UnconfiguredAliasIsNotAttested`)
- **A client disconnect is no longer reported as a stream timeout.** The disconnect watcher watched the same derived context as the total-stream deadline, so two `select` cases were ready on one signal and the outcome was chosen at random. Everything after the stream loop then recorded HTTP 200 and a usage row for timeouts and disconnects alike. `StreamMetrics.Outcome` now records which of the six exits ran. (`internal/gateway/streaming_enhanced.go`)
...
### Changed
- **`GET /aegis/v1/audit/logs` returns 410 Gone.** Nothing has ever written `audit_logs`, so the endpoint returned an empty list for its whole existence while emitting a full 21-column CSV header, which read as a working feature reporting that nothing had happened. The body points at `/aegis/v1/audit/events`. The table is retained: `internal/purge` targets it by name and two guards still sweep it. Removal is tracked in `docs/evidence/known-limitations.md` §2.11. (`internal/gateway/audit_handler.go`)
- **A provider error body is never logged verbatim.** Both the streaming and non-streaming paths now log the status, request id, configured provider key and a bounded redacted excerpt. A provider that rejects a request commonly quotes the request back, and what it quotes back can be the credential that failed, which the gateway refused to forward on the way out and would then have written to its own logs on the way back. `adapters.RedactProviderError` replaces secret spans, collapses control characters so an embedded newline cannot forge a JSON log record, and truncates to 256 bytes. (`internal/router/adapters/provider_error.go`)
