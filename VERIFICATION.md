...
| "**OpenAI-compatible API** — drop-in replacement; no SDK changes required" | `README.md:160` | [`internal/types/request.go`](https://github.com/aegis-gateway/aegis-ai-gateway/blob/ea72971186eb5c316966b065bf710f2d85f578b1/internal/types/request.go) — `AegisRequest` has no `Tools`, `ToolChoice`, `Functions`, or `FunctionCall` fields; they are silently stripped on ingress | **WRONG — tool calling stripped**. Corrected in `README.md` to: "drop-in replacement for chat completions; `tools` and function calling are not yet supported and are stripped on ingress" |
